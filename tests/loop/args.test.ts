...
test("parseArgs prints version and exits when --version is passed", () => {
  ...
  console.log = ((_: string) => {
    expect(_).toBe(`Proofloop v${LOOP_VERSION}`);
...
test("parseArgs returns expected defaults when proof is omitted", () => {
  ...
  expect(opts.qualityGate).toBe("adversarial");
...
test("parseArgs handles all value flags and explicit reviewer", () => {
  ...
  expect(opts.format).toBe("pretty");
  expect(opts.qualityGate).toBe("none");
  expect(opts.review).toBe("claudex");
  expect(opts.codexModel).toBe("custom-model");
...
test("parseArgs accepts and validates --quality-gate", () => {
  expect(parseArgs(["--quality-gate", "none"]).qualityGate).toBe("none");
  expect(parseArgs(["--quality-gate", "adversarial"]).qualityGate).toBe(
    "adversarial"
  );
  expect(() => parseArgs(["--quality-gate", "banana"])).toThrow(
    "Invalid --quality-gate value: banana"
  );
});
