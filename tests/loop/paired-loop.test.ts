...
import { REVIEW_PASS } from "../../src/loop/constants";
...
let importNonce = 0;
let qualityGateImpl = async (): Promise<QualityGateResult> => ({
  approved: true,
  commandResults: [],
  notes: "",
});
...
const loadPairedLoop = async (): Promise<PairedLoopModule> => {
  mock.restore();
  ...
  module.pairedLoopInternals.setDeps({
    createRunQualityGate: () => (_reviewers, _task, opts) => {
      if (opts.qualityGate !== "adversarial") {
...
afterEach(() => {
  ...
  readlineAnswers = [];
  readlinePrompts = [];
  qualityGateImpl = async (): Promise<QualityGateResult> => ({
    approved: true,
    commandResults: [],
    notes: "",
  });
...
test("runPairedLoop records quality gate transcript evidence before completion", async () => {
  const module = await loadPairedLoop();
  runAgentImpl = (_agent, prompt) => {
    if (prompt.includes("Review this completed work")) {
      return Promise.resolve(makeResult(REVIEW_PASS));
    }
  ...
  qualityGateImpl = async (): Promise<QualityGateResult> => ({
    ...
    commandResults: [
      {
        command: { args: ["bun", "test"], name: "tests" },
        exitCode: 0,
        output: "ok",
      },
    ],
    ...
    review: {
      ...
      failures: [],
      notes: "",
      reviews: [{ reason: "", reviewer: "codex", status: "pass" }],
    },
  });

  await withTempHome("quality", async (runDir) => {
    ...
    expect(transcript).toContain('"kind":"quality-gate"');
    expect(transcript).toContain('"command":"bun test"');
    expect(transcript).toContain('"scope":"quality"');
  });
});
