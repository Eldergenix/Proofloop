...
const asReadableStream = (
  ...
  throw new Error(`claude sdk ${name} stream is unavailable`);
};

const drainStream = (stream: BunReadableStream): void => {
...
const pipeToStderr = (stream: BunReadableStream): void => {
  const reader = stream.getReader();
  const pump = (): void => {
    reader
      .read()
...
class ClaudeSdkClient {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      drainStream(asReadableStream(this.child.stdout, "stdout"));
      pipeToStderr(asReadableStream(this.child.stderr, "stderr"));
