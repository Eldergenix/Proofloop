...
import { createRunQualityGate, type QualityGateResult } from "./quality-gate";
import { createRunReviewWithPrompt, resolveReviewers } from "./review";
import {
  appendRunTranscriptEntry,
  createRunQualityGateEntry,
...
const QUALITY_GATE_FAILURE_LOG =
...
interface PairedLoopDependencies {
  createRunQualityGate: typeof createRunQualityGate;
...
const appendTranscript = (
  ...
  entry:
    ...
    | ReturnType<typeof createRunQualityGateEntry>
...
const appendQualityGateTranscript = (
  state: PairedState,
  ...
  appendTranscript(
    state,
    createRunQualityGateEntry(
      qualityGate.approved ? "pass" : "fail",
      qualityGate.commandResults,
      qualityGate.notes
    )
  );
  for (const outcome of qualityGate.review?.reviews ?? []) {
    appendTranscript(
      state,
      createRunReviewEntry(
        outcome.reviewer,
...
const createPairedQualityGate = (state: PairedState) =>
  pairedLoopDeps.createRunQualityGate({
    runReviewer: (reviewer, prompt, opts) =>
      runReviewerAgent(reviewer, prompt, opts, nextResumeId(state, reviewer)),
  });
...
const handleDoneSignal = async (
  ...
  runQualityGate: ReturnType<typeof createPairedQualityGate>
  ...
  if (reviewers.length === 0) {
    const qualityGate = await runQualityGate([], task, state.options);
    updateIds(state);
    appendQualityGateTranscript(state, qualityGate);
    if (!qualityGate.approved) {
      transitionRunState(state, "working", "quality gate requested changes");
      console.log(QUALITY_GATE_FAILURE_LOG);
      return { done: false, reviewNotes: qualityGate.notes };
    }
  ...
  if (review.approved) {
    const qualityGate = await runQualityGate(reviewers, task, state.options);
    updateIds(state);
    appendQualityGateTranscript(state, qualityGate);
    if (!qualityGate.approved) {
      transitionRunState(state, "working", "quality gate requested changes");
    ...
    console.log(
      `\n[loop] ${doneText(state.options.doneSignal)} detected, review passed, and quality gate passed, stopping.`
    );
    return { done: true, reviewNotes: "" };
  }
...
const runIterations = async (
  ...
  const runQualityGate = createPairedQualityGate(state);
  console.log(`\n[loop] PLAN.md:\n\n${task}`);
