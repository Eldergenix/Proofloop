...
const parseQualityGateValue = (value: string): QualityGateMode => {
  if (value === "adversarial" || value === "none") {
    return value;
  }
  throw new Error(`Invalid --quality-gate value: ${value}`);
};
...
const applyValueFlag = (
  ...
  switch (flag) {
    ...
    case "format":
      ...
      return;
    case "qualityGate":
      opts.qualityGate = parseQualityGateValue(value);
      return;
    default: {
...
const consumeArg = (
  ...
  if (arg === "-v" || arg === "--version") {
    console.log(`Proofloop v${LOOP_VERSION}`);
    process.exit(0);
  }
