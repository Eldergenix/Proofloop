...
const isReviewScope = (value: string): value is ReviewScope =>
  value === "standard" || value === "quality";
...
export type ReviewScope = "standard" | "quality";

export interface RunQualityGateCommandTranscriptEntry {
  command: string;
  exitCode: number;
  name: string;
  output?: string;
...
export interface RunQualityGateTranscriptEntry {
  at: string;
  commands: RunQualityGateCommandTranscriptEntry[];
  kind: "quality-gate";
  reason?: string;
  status: ReviewStatus;
}
...
export const createRunReviewEntry = (
  ...
  ...(reason ? { reason } : {}),
  reviewer,
  ...(scope && scope !== "standard" ? { scope } : {}),
  status,
});

export const createRunQualityGateEntry = (
  status: ReviewStatus,
  results: Array<{
    command: { args: string[]; name: string };
    exitCode: number;
    output?: string;
  }>,
  reason?: string,
...
const parseReviewTranscriptEntry = (
  ...
  const scope =
    typeof parsed.scope === "string" && isReviewScope(parsed.scope)
      ? parsed.scope
      : undefined;
  const status =
...
const parseQualityGateCommand = (
  value: unknown
): RunQualityGateCommandTranscriptEntry | undefined => {
  if (!isRecord(value)) {
    return undefined;
  }
  const command = asString(value.command);
  const name = asString(value.name);
  const exitCode = asInteger(value.exitCode);
  if (!(command && name && exitCode !== undefined)) {
    return undefined;
  }
...
const parseQualityGateTranscriptEntry = (
  parsed: Record<string, unknown>,
  at: string
): RunQualityGateTranscriptEntry | undefined => {
  const status =
    typeof parsed.status === "string" && isReviewStatus(parsed.status)
      ? parsed.status
      : undefined;
  ...
  const commands = rawCommands
    .map((command) => parseQualityGateCommand(command))
...
const parseRunTranscriptEntry = (
  ...
  if (kind === "quality-gate") {
    return parseQualityGateTranscriptEntry(parsed, at);
  }
  if (kind === "result") {
